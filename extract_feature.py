...
if openStep1:
    ...
    off_train = pd.read_csv('input/ccf_offline_stage1_train.csv')
    ...
    off_test = pd.read_csv('input/ccf_offline_stage1_test_revised.csv')
    off_test.columns = ['user_id','merchant_id','coupon_id','discount_rate','distance','date_received']

    dataset3 = off_test

    feature3 = off_train[((off_train.date>='20160319')&(off_train.date<='20160630'))|((off_train.date=='null')&(off_train.date_received>='20160319')&(off_train.date_received<='20160630'))]
    dataset2 = off_train[(off_train.date_received>='20160515')&(off_train.date_received<='20160614')]
    feature2 = off_train[(off_train.date>='20160201')&(off_train.date<='20160514')|((off_train.date=='null')&(off_train.date_received>='20160201')&(off_train.date_received<='20160514'))]
    dataset1 = off_train[(off_train.date_received>='20160414')&(off_train.date_received<='20160514')]
    feature1 = off_train[(off_train.date>='20160101')&(off_train.date<='20160413')|((off_train.date=='null')&(off_train.date_received>='20160101')&(off_train.date_received<='20160413'))]
    feature3.to_pickle("data/feature/f3.pkl")
    feature2.to_pickle("data/feature/f2.pkl")
    feature1.to_pickle("data/feature/f1.pkl")
    dataset3.to_pickle("data/train/d3.pkl")
    dataset2.to_pickle("data/train/d2.pkl")
    dataset1.to_pickle("data/train/d1.pkl")
else :
    feature3 = pd.read_pickle("data/feature/f3.pkl")
    feature2 = pd.read_pickle("data/feature/f2.pkl")
    feature1 = pd.read_pickle("data/feature/f1.pkl")
    dataset3 = pd.read_pickle("data/train/d3.pkl")
    dataset2 = pd.read_pickle("data/train/d2.pkl")
    dataset1 = pd.read_pickle("data/train/d1.pkl")

if openOnlineStep:
    # off_train = pd.read_csv('input/ccf_offline_stage1_train.csv')
    # off_train.columns = ['user_id', 'merchant_id', 'coupon_id', 'discount_rate', 'distance', 'date_received', 'date']
    # 11429826 record(872357 with coupon_id),762858 user(267448 in off_train)
    on_train = pd.read_csv('input/ccf_online_stage1_train.csv')
    on_train.columns = ['user_id', 'merchant_id', 'action', 'coupon_id', 'discount_rate', 'date_received', 'date']

    t = on_train[['user_id','merchant_id']]
    t.is_copy = False
    t.drop_duplicates(inplace=True)
    print("start")
    t1 = on_train[on_train.action==0][['user_id','merchant_id']]
    t1['user_some_merchant_on_click'] = 1
    t1 = t1.groupby(["user_id","merchant_id"]).agg("sum").reset_index()
    print(t1.shape) #(3331740, 3)
    t2 = on_train[on_train.action==1][['user_id','merchant_id']]
    t2['user_some_merchant_on_buy'] = 1
    t2 = t2.groupby(["user_id","merchant_id"]).agg("sum").reset_index()
    print(t2.shape) #(686054, 3)
    t3 = on_train[on_train.action == 2][['user_id', 'merchant_id']]
    t3['user_some_merchant_on_coupon']= 1
    t3 = t3.groupby(["user_id", "merchant_id"]).agg("sum").reset_index()
    print(t3.shape) #(324069, 3)
    online_feature = pd.merge(t,t1, on=['user_id','merchant_id'],how='left')
    online_feature = pd.merge(online_feature, t2, on=['user_id','merchant_id'],how='left')
    online_feature = pd.merge(online_feature, t3, on=['user_id','merchant_id'], how='left')
    online_feature.to_pickle("data/online_feature.pkl")
elif openOnlineCal:
    online_feature = pd.read_pickle("data/online_feature.pkl")
    online_feature['user_some_merchant_on_coupon_transfer_rate'] = online_feature.user_some_merchant_on_coupon / online_feature.user_some_merchant_on_buy
    # online_feature[
    #     'user_some_merchant_on_click_transfer_rate'] = online_feature.user_some_merchant_on_click / online_feature.user_some_merchant_on_coupon
    online_feature = online_feature.groupby(['user_id']).agg(
        {'user_some_merchant_on_coupon_transfer_rate': ['mean'],
         #'user_some_merchant_on_click_transfer_rate': ['mean','size'],
         'user_some_merchant_on_coupon':['mean']
         })
    online_feature.columns = online_feature.columns.droplevel(0)
    online_feature.columns = [
        'user_merchant_on_coupon_transfer_rate_mean',
        #'user_merchant_on_click_transfer_rate_mean','user_merchant_on_click_transfer_rate_size',
        'user_some_merchant_on_coupon_mean'
    ]
    online_feature.reset_index(inplace=True)
    online_feature.user_merchant_on_coupon_transfer_rate_mean = online_feature.user_merchant_on_coupon_transfer_rate_mean.replace(np.nan,0)
    #online_feature.user_merchant_on_coupon_transfer_rate_size = online_feature.user_merchant_on_coupon_transfer_rate_size .replace(np.nan,0)
    #online_feature.user_merchant_on_click_transfer_rate_mean = online_feature.user_merchant_on_click_transfer_rate_mean.replace(np.nan,0)
    #online_feature.user_merchant_on_click_transfer_rate_size = online_feature.user_merchant_on_click_transfer_rate_size.replace(np.nan,0)
    online_feature.user_some_merchant_on_coupon_mean = online_feature.user_some_merchant_on_coupon_mean.replace(np.nan,0)
    #online_feature.user_some_merchant_on_coupon_size = online_feature.user_some_merchant_on_coupon_size .replace(np.nan,0)
    ...
    #online_feature.loc[:,'hasOnlineRate'] = 0
    #online_feature.hasOnlineRate = online_feature.user_merchant_on_coupon_transfer_rate_mean.apply(hasOnlineRate)
    #online_feature.to_pickle("data/online_feature_agg.pkl")
...
if openStep2:
    ...
    t1 = t1.groupby(['user_id','coupon_id']).agg('sum').reset_index()   #領取次數

    t2 = dataset3[['user_id','coupon_id','date_received']]
    t2.date_received = t2.date_received.astype('str')
    t2 = t2.groupby(['user_id','coupon_id'])['date_received'].agg(lambda x:':'.join(x)).reset_index()
    t2['receive_number'] = t2.date_received.apply(lambda s:len(s.split(':')))   #領取次數
    ...
    #t3.date_received = pd.to_numeric(t3.date_received, errors='coerce')
    ...
    t4.loc[:, 'this_day_user_receive_all_coupon_count'] = 1
    t4 = t4.groupby(['user_id','date_received']).agg('sum').reset_index()

    t5 = dataset3[['user_id','coupon_id','date_received']]
    t5.loc[:, 'this_day_user_receive_same_coupon_count'] = 1
    ...
    if open20171105 and 0:
        t7['day_gap_after_15'] = t7.day_gap_after.apply(isIn15Days)  ##20171105
        t7 = t7[['user_id','coupon_id','date_received','day_gap_before','day_gap_after','day_gap_after_15']]
    else:
...
if openStep4:
    ...
    if open20171105:
        #20171105#
        user3 = feature3[['user_id', 'merchant_id', 'date', 'date_received']]
        user3 = user3[(user3.date != 'null') & (user3.date_received != 'null')]
        user3['merchant_date_datereceived_gap'] = user3.date + ':' + user3.date_received
        user3.merchant_date_datereceived_gap = user3.merchant_date_datereceived_gap.apply(get_user_date_datereceived_gap)

        t9 = user3[['user_id', 'merchant_id']]
        t9.loc[:,'user_merchant_buy_use_coupon'] = 1
        t9 = t9.groupby(['user_id', 'merchant_id']).agg('sum').reset_index()
        t9.drop_duplicates(inplace=True)
        t10 = t9[['merchant_id', 'user_merchant_buy_use_coupon']]
        t10 = t10.groupby(["merchant_id"]).agg(
            {'user_merchant_buy_use_coupon': ['mean']
             })
        t10.columns = t10.columns.droplevel(0)
        t10.columns = [
            'user_merchant_buy_use_coupon_mean'
            #'user_some_merchant_on_coupon_median'
        ]
        t10.reset_index(inplace=True)

        t11 = user3[['merchant_id', 'merchant_date_datereceived_gap']]
        t12 = t11.groupby('merchant_id').agg('mean').reset_index()
        t12.rename(columns={'merchant_date_datereceived_gap': 'avg_merchant_date_datereceived_gap'}, inplace=True)
        t13 = t11.groupby('merchant_id').agg('min').reset_index()
        t13.rename(columns={'merchant_date_datereceived_gap': 'min_merchant_date_datereceived_gap'}, inplace=True)
        t14 = t11.groupby('merchant_id').agg('max').reset_index()
        t14.rename(columns={'merchant_date_datereceived_gap': 'max_merchant_date_datereceived_gap'}, inplace=True)
        # 20171105#
    ...
    if open20171105:
        merchant3_feature = pd.merge(merchant3_feature, t10, on='merchant_id', how='left') # 20171105#
        merchant3_feature = pd.merge(merchant3_feature, t12, on='merchant_id', how='left')  # 20171105#
        merchant3_feature = pd.merge(merchant3_feature, t13, on='merchant_id', how='left')  # 20171105#
        merchant3_feature = pd.merge(merchant3_feature, t14, on='merchant_id', how='left')  # 20171105#
    ...
    if open20171105:
    # 20171105#
        user2 = feature2[['user_id', 'merchant_id', 'date', 'date_received']]
        user2 = user2[(user2.date != 'null') & (user2.date_received != 'null')]
        user2['merchant_date_datereceived_gap'] = user2.date + ':' + user2.date_received

        user2.merchant_date_datereceived_gap = user2.merchant_date_datereceived_gap.apply(get_user_date_datereceived_gap)
        t9 = user2[['user_id', 'merchant_id']]
        t9.loc[:, 'user_merchant_buy_use_coupon'] = 1
        t9 = t9.groupby(['user_id', 'merchant_id']).agg('sum').reset_index()
        t9.drop_duplicates(inplace=True)
        t10 = t9[['merchant_id', 'user_merchant_buy_use_coupon']]
        ...
        t11 = user2[['merchant_id', 'merchant_date_datereceived_gap']]
        t12 = t11.groupby('merchant_id').agg('mean').reset_index()
        t12.rename(columns={'merchant_date_datereceived_gap': 'avg_merchant_date_datereceived_gap'}, inplace=True)
        t13 = t11.groupby('merchant_id').agg('min').reset_index()
        t13.rename(columns={'merchant_date_datereceived_gap': 'min_merchant_date_datereceived_gap'}, inplace=True)
    ...
    if open20171105:
        merchant2_feature = pd.merge(merchant2_feature, t10, on='merchant_id', how='left')  # 20171105#
        merchant2_feature = pd.merge(merchant2_feature, t12, on='merchant_id', how='left')  # 20171105#
        merchant2_feature = pd.merge(merchant2_feature, t13, on='merchant_id', how='left')  # 20171105#
        merchant2_feature = pd.merge(merchant2_feature, t14, on='merchant_id', how='left')  # 20171105#
    ...
    if open20171105:
        # 20171105#
        user1 = feature1[['user_id', 'merchant_id', 'date', 'date_received']]
        user1 = user1[(user1.date != 'null') & (user1.date_received != 'null')]
        user1['merchant_date_datereceived_gap'] = user1.date + ':' + user1.date_received
        user1.merchant_date_datereceived_gap = user1.merchant_date_datereceived_gap.apply(get_user_date_datereceived_gap)
        t9 = user1[['user_id', 'merchant_id']]
        t9['user_merchant_buy_use_coupon'] = 1
        t9 = t9.groupby(['user_id', 'merchant_id']).agg('sum').reset_index()
        t9.drop_duplicates(inplace=True)
        t10 = t9[['merchant_id', 'user_merchant_buy_use_coupon']]
        t10 = t10.groupby(["merchant_id"]).agg(
        ...
        t11 = user1[['merchant_id', 'merchant_date_datereceived_gap']]
        t12 = t11.groupby('merchant_id').agg('mean').reset_index()
        t12.rename(columns={'merchant_date_datereceived_gap': 'avg_merchant_date_datereceived_gap'}, inplace=True)
        t13 = t11.groupby('merchant_id').agg('min').reset_index()
        t13.rename(columns={'merchant_date_datereceived_gap': 'min_merchant_date_datereceived_gap'}, inplace=True)
    ...
    if open20171105:
        merchant1_feature = pd.merge(merchant1_feature, t10, on='merchant_id', how='left')  # 20171105#
        merchant1_feature = pd.merge(merchant1_feature, t12, on='merchant_id', how='left')  # 20171105#
        merchant1_feature = pd.merge(merchant1_feature, t13, on='merchant_id', how='left')  # 20171105#
        merchant1_feature = pd.merge(merchant1_feature, t14, on='merchant_id', how='left')  # 20171105#
...
if openStep6:
    ...
    all_user_merchant.is_copy = False
...
if openOnlineCal:
    dataset3 = pd.merge(dataset3,online_feature,on=['user_id'],how='left')
    dataset3.loc[:,'hasOnlineRate'] = 0
    dataset3.hasOnlineRate = dataset3.user_merchant_on_coupon_transfer_rate_mean.apply(hasOnlineRate)
...
if openOnlineCal:
    dataset2 = pd.merge(dataset2,online_feature,on=['user_id'],how='left')
    dataset2.loc[:, 'hasOnlineRate'] = 0
    dataset2.hasOnlineRate = dataset2.user_merchant_on_coupon_transfer_rate_mean.apply(hasOnlineRate)
...
if openOnlineCal:
    dataset1 = pd.merge(dataset1,online_feature,on=['user_id'],how='left')
    dataset1.loc[:, 'hasOnlineRate'] = 0
    dataset1.hasOnlineRate = dataset1.user_merchant_on_coupon_transfer_rate_mean.apply(hasOnlineRate)
